...
A capacitor is a device that stores charge. If while charging, the total change in voltage is $\Delta V$ and starting voltage is $V(0)=V_0$, then the voltage at time $t$ is found with

\begin{equation}
        \label{cap}
        V\left(t\right)=V_0+\Delta V\left(1-e^{-\frac{t}{T}}\right)
\end{equation}
...
\begin{equation}
        \label{vplus}
        V_+(t)  =   \frac{1}{2} V_\text{out}(t)
\end{equation}

The interesting physics happens in the negative feedback ($V_-$), connected to a capacitor-resistor circuit with exponential charge/discharge. When the capacitor drops below the critical threshold $V_-<V_+$, the amplified output from Eq~\eqref{opamp} will immediately create a positive voltage and force a charge cycle. Likewise, when the capacitor charges above $V_->V_+$, a negative voltage will force a discharge cycle. In either case, the op-amp will quickly reach  maximum saturation voltage $V_\text{out}=\pm V_S$, which will be the amplitude of our wave-form.

Solving $V_-=V_+$ at $V_\text{out}=V_S$ can give us the "flipping" time, which should be a half-period of the oscillation. By Eq~\eqref{vplus}, the positive feedback is constrained by $\pm V_+(t) = \pm V_S / 2$ at saturation. This must also be the negative feedback constraint, since the op-amp is working to equalize the two inputs. Let $t$ be the time for the capacitor to go from $V_-=-V_S / 2$ to $V_-=+V_S / 2$.

\begin{align*}
        \frac{V_S }{2} & = -\frac{V_S}{2}+V_S(1-e^{(-t/ \tau)}) \\[0.5em]
        \frac{1}{2}    & = 1-2e^{-t/\tau}
\end{align*}
